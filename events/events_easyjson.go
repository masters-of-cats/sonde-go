...
func easyjson692db02bDecodeGithubComCloudfoundrySondeGoEvents7(in *jlexer.Lexer, out *ContainerMetric) {
	...
	for !in.IsDelim('}') {
		...
		case "cpuPercentageWeighted":
			if in.IsNull() {
				in.Skip()
				out.CpuPercentageWeighted = nil
			} else {
				if out.CpuPercentageWeighted == nil {
					out.CpuPercentageWeighted = new(float64)
				}
				*out.CpuPercentageWeighted = float64(in.Float64())
			}
		default:
			in.SkipRecursive()
		}
...
func easyjson692db02bEncodeGithubComCloudfoundrySondeGoEvents7(out *jwriter.Writer, in ContainerMetric) {
	...
	if in.CpuPercentageWeighted != nil {
		const prefix string = ",\"cpuPercentageWeighted\":"
		if first {
			first = false
			out.RawString(prefix[1:])
		} else {
		...
		out.Float64(float64(*in.CpuPercentageWeighted))
	}
	out.RawByte('}')
}
